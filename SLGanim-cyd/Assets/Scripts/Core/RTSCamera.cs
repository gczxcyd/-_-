...
public class RTSCamera : MonoBehaviour
{
    ...
    public void RotateCamera(bool left)
    {
        if(cameraState == CameraState.idle)
        {
            ...
            if (left)
            {
                anchor.transform.DORotate(new Vector3(0, anchor.transform.rotation.eulerAngles.y + 90, 0), 1.0f).OnPlay(() => {
            ...
            {
                anchor.transform.DORotate(new Vector3(0, anchor.transform.eulerAngles.y - 90, 0), 1.0f).OnPlay(() => {
                    cameraState = CameraState.rotate;
                }).OnComplete(() => {
                    cameraState = CameraState.idle;
                    transform.SetParent(null);
